...
# Global parameters
src_table_name = 'item_flags'
src_schema_name = 'public'

tgt_table_name = 'mrr_item_flags_tag'
tgt_schema_name = 'mrr'

dwh_tgt_incremental_column = 'updated_at'
dwh_tgt_table_name = 'ods_item_flags'
dwh_tgt_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = '/etl/mrr_item_flags_tag.csv'

file_path = '//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_11//increment//mrr_item_flags_tag.csv'
...
def read_redshift_incremental_param(schema_name, table_name, incremental_column, v_engine):
    ...
    # check null
    if (len(incremental_date) < 10 or incremental_date is None or incremental_date == "" or incremental_date == '""') and (table_name in table_list):
        sql_none = "select  date_trunc('day',dateadd(day,-5,getdate())) as max_admin_creation_time"
        data = pd.read_sql_query(sql_none, v_engine)
        incremental_date = data["max_admin_creation_time"].to_csv(None, header=False, encoding="utf-8", index=False)
    if (len(incremental_date) < 10 or incremental_date is None or incremental_date == "" or incremental_date == '""') and (table_name not in table_list):
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            created_at,
            updated_at,
            evaluation_bid_timeout,
            partner_transfered_funds_note,
            shipment_call_user,
            shipment_call_user_note,
            shipment_call_user_satisfaction,
            transfer_call_user,
            transfer_call_user_note,
            transfer_call_user_satisfaction,
            broker_id,
            external_care,
            shipped_via,
            owned_by_worthy,
            cloned_item_id,
            source_item_id,
            evaluation_matches_estimation,
            trade_in_deal_type,
            chat_bot,
            submitted_from_device,
            unmount_stone_approval_method,
            tradein_low_value,
            tradein_low_value_threshold,
            full_diamond_description,
            seller_notes,
            buy_it_now_price,
            last_auction_started_by,
            recommended_minimum_price,
            auto_estimated,
            automatically_auctioned_subsets,
            unmount_approval_by,
            auto_select_winning_bid,
            high_value_page_displayed,
            high_value_user_phone_changed,
            high_value_scheduled_appointment,
            canceled_unmount_approval,
            no_estimation_displayed,
            generated_marketplace_photos_from_diacam,
            advance_payment,
            sticky_auction,
            submission_type,
            format,
            ...
            new_auction,
            new_auction_tiers,
            tiers_update_august_2019,
            price_guarantee,
            second_item,
            tiers_update_january_2020,
            new_auction_2020,
            flow,
            photo_validation_review,
            photo_validation_comment,
            personal_reminder_at,
            partner_transferred_funds,
            listing_trends,
            light_trends_presented,
            creation_google_cid,
            prereg_score,
            creation_session_identifier
            FROM {0}""".format(schema_table_name.replace('\'', '\'\''))
    print(sql)
    start = 0
    dfs = []
    # clear file if exist
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                creation_session_identifier
                FROM {0}.{1} 
                WHERE updated_at > '{2}'""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
...
def df_to_s3(v_bucket, v_s3_file, data_frame):
    data = data_frame
    bytes_to_write = data.to_csv(None, header=False, encoding="utf-8", index=False, sep=';').encode()
