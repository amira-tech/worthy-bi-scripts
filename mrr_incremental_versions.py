...
# Global parameters
src_table_name = "versions"
src_schema_name = "public"

tgt_table_name = "mrr_versions_tag"
tgt_schema_name = "mrr"

dwh_tgt_incremental_column = "created_at"
dwh_tgt_table_name = "ods_versions"
dwh_tgt_schema_name = "public"

bucket_name = "worthy-etl"
s3_file_name = "/etl/mrr_versions_tag.csv"

file_path = "//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_1//increment//mrr_versions_tag.csv"
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            created_at,
            object_changes
            FROM {0} WHERE object_changes like '%reject_reason%' and item_type = 'Item' AND event = 'update'""".format(schema_table_name.replace('\'', '\'\''))
    print(sql)
    start = 0
    dfs = []
    # clear file if exist
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                object_changes
                FROM {0}.{1} 
                WHERE created_at > '{2}' and object_changes like '%reject_reason%' and item_type = 'Item' AND event = 'update'""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
