...
## global params
src_table_name = 'histories'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_full_first_estimation_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_full_first_estimation_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_first_estimation_tag'
tgt_schema_name = 'mrr'
...
def read_sql_max_rows(schema_table_name, conn):
    # sql max
    sql_max = "select count(*) as rows_count from (select item_id,replace(substring(comment,'\$(.+)'),CHR(44),'')::float first_estimation,row_number() over(partition by item_id order by created_at) r from histories where from_module=1 and from_submodule=3 and historiable_type='Item' and comment like 'Estimation set:%') a where r=1"
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = "select item_id, first_estimation from (select item_id,replace(substring(comment,'\$(.+)'),CHR(44),'')::float first_estimation,row_number() over(partition by item_id order by created_at) r from histories where from_module=1 and from_submodule=3 and historiable_type='Item' and comment like 'Estimation set:%') a where r=1"
    start = 0
    dfs = []
    # clear file if exist
    try:
