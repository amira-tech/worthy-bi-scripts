...
## global params
src_table_name = 'property_options'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_property_options_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_property_options_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_property_options_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = "SELECT id ,property_id	,description ,created_at ,updated_at ,priority ,subset_type_id ,frontend_display ,system_name ,stores_display ,quality FROM {} ".format(str(schema_table_name))
    start = 0
    dfs = []
    # clear file if exist
    try:
