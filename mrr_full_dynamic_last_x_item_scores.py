...
## global params
src_table_name = 'dynamic_last_x_item_scores'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_full_dynamic_last_x_item_scores_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_full_dynamic_last_x_item_scores_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_dynamic_last_x_item_scores_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = "SELECT id,user_id,item_id, item_age_days,event_created_date,	static_anomaly_score, static_model_id, static_score, is_item_arrived, fixed_age_p_dynamic_qual2deliver_eventually,fixed_age_dynamic_score, dynamic_model_id,	dynamic_model_predict_date,	created_at,	age, active, has_ga_detailed FROM {} WHERE active = 'true'".format(str(schema_table_name))
    start = 0
    dfs = []
    # clear file if exist
    try:
