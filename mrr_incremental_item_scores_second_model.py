...
tgt_table_name = 'mrr_item_scores_second_model_tag'
tgt_schema_name = 'mrr'

dwh_tgt_incremental_column = 'score_model_id'
dwh_tgt_table_name = 'ods_item_scores'
dwh_tgt_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = '/etl/mrr_user_experiments_tag.csv'

file_path = '//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_11//increment//mrr_user_experiments_tag.csv'
...
def read_redshift_incremental_param(schema_name, table_name, incremental_column, v_engine):
    sql = 'SELECT max(' + incremental_column + '::integer) - 1 as max_admin_creation_time FROM ' + schema_name + '.' + table_name + ' da '
    ...
    # check null
    if (incremental_date is None or incremental_date == "" or incremental_date == '""') :
        sql_none = "select 0"
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            expected_gmv,
            p_time2deliver,
            p_qual2deliver_1month,
            has_rockprop,
            p_qual2deliver_2weeks,
            p_qual2deliver_auctionable,
            p_qual2deliver_auctionable_2weeks,
            p_qual2deliver_auctionable_1month,
            has_ga_detailed,
            p_deliver_auctionable2deal   
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                p_deliver_auctionable2deal   
                FROM {0}.{1} 
                WHERE score_model_id::integer = '{2}'::integer""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
