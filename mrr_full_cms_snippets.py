...
## global params
src_table_name = 'cms_snippets'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_cms_snippets_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_cms_snippets_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_cms_snippets_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = r"SELECT id,site_id,""label"",identifier,""content"",""position"",is_shared,created_at,updated_at FROM cms_snippets WHERE identifier ilike '%temp%' and site_id=228" 
    start = 0
    dfs = []
    # clear file if exist
    try:
