...
## global params
src_table_name = 'admins'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_admins_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_admins_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_admins_tag'
tgt_schema_name = 'mrr'
...
def read_sql_max_rows(schema_table_name, conn):
    # sql max
    sql_max = f'SELECT count(*) as rows_count from {schema_table_name}'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = f'SELECT id, email, encrypted_password, sign_in_count, current_sign_in_at, last_sign_in_at, current_sign_in_ip, last_sign_in_ip, created_at, updated_at, "name", "role", disabled, image, autoassign, local_time_zone, weight_category, post_operator_id, calendar_id, bot, last_assigned, backend_access, incontact_username, show_on_scoreboard, move_leads_to_bot, move_leads_automatically, bot_type, zendesk_username, show_on_wilds, is_jb, full_life_cycle, provider, uid, office, deputy_id, role_management_id, phone_extension from {schema_table_name}'
    start = 0
    dfs = []
    # clear file if exist
    try:
