...
file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_tasks_tag.csv'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = f'SELECT id,taskable_id,taskable_type,task_type_id,created_by_id,created_by_type,assignee_id,"desc",state,done_by_user_id,due_date,state_changed_at,created_at,updated_at,assignee_type,delete_on,user_id,user_type,priority,ate_list_id,assigned_by_id,assigned_by_type,estimation,follow_up from {schema_table_name}'
    start = 0
    dfs = []
    # clear file if exist
    try:
