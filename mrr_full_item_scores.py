...
## global params
src_table_name = 'item_scores'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_full_item_scores_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_full_item_scores_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_item_scores_tag'
tgt_schema_name = 'mrr'

schema_table_name = src_schema_name + '.' + src_table_name
...
def read_sql_max_rows(schema_table_name, conn):
    sql_max = "SELECT count(*) as rows_count FROM {} WHERE active = 'true'".format(str(schema_table_name))
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = "SELECT id,created_at,updated_at,item_id,user_id,item_estimation_amount,score_model_id,has_ga,p_qual2deliver,p_deliver2deal,p_qual2deal,score,anomaly_score,active,rake_task_event_id,p_estieval,expected_gmv,p_time2deliver,p_qual2deliver_1month,has_rockprop,p_qual2deliver_2weeks,p_qual2deliver_auctionable,p_qual2deliver_auctionable_2weeks,p_qual2deliver_auctionable_1month,has_ga_detailed,p_deliver_auctionable2deal FROM {} WHERE active = 'true'".format(str(schema_table_name))
    start = 0
    dfs = []
    # clear file if exist
    try:
...
if __name__ == '__main__':
    ...
    print("")
    max_so = read_sql_max_rows(schema_table_name, connection)
    print('source_max_row: ' + str(max_so))
    read_sql_chunked(src_schema_table_name, connection, 10000 * 4, 15000, file_path)
    datetime_object = datetime.now()
