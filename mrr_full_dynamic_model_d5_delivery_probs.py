...
## global params
src_table_name = 'dynamic_model_d5_delivery_probs'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_full_dynamic_model_d5_delivery_probs_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_full_dynamic_model_d5_delivery_probs_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_dynamic_model_d5_delivery_probs_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = "SELECT id,item_id,deliver_probability,item_age_days,static_model_id,dynamic_model_id,model_run_date,conversion_event_sent,created_at,updated_at FROM {} ".format(str(schema_table_name))
    start = 0
    dfs = []
    # clear file if exist
    try:
