...
# Global parameters
src_table_name = 'user_account_managers'
src_schema_name = 'public'

tgt_table_name = 'mrr_user_account_managers_tag'
tgt_schema_name = 'mrr'

dwh_tgt_incremental_column = 'start_time'
dwh_tgt_table_name = 'ods_user_account_managers'
dwh_tgt_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = '/etl/mrr_user_account_managers_tag.csv'

file_path = '//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_11//increment//mrr_user_account_managers_tag.csv'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            start_time,
            end_time,
            never_contacted_on_assignment,
            never_contacted_on_unassignment,
            allocation_type_id,
            rake_task_event_id,
            history_id
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                history_id
                FROM {0}.{1} 
                WHERE start_time > '{2}'""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
