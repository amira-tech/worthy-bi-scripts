...
tgt_table_name = 'mrr_tags_tag'
tgt_schema_name = 'mrr'

dwh_tgt_incremental_column = 'updated_at'
dwh_tgt_table_name = 'ods_tags'
dwh_tgt_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = '/etl/mrr_tags_tag.csv'

file_path = '//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_11//increment//mrr_tags_tag.csv'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            created_at,
            updated_at
            FROM {0} WHERE id != 2143381 """.format(schema_table_name.replace('\'', '\'\''))
    print(sql)
    start = 0
    dfs = []
    # clear file if exist
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                updated_at
                FROM {0}.{1} 
                WHERE updated_at > '{2}' AND id != 2143381""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
