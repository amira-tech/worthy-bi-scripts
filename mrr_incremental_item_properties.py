...
# Global parameters
src_table_name = 'item_properties'
src_schema_name = 'public'

tgt_table_name = 'mrr_item_properties_tag'
tgt_schema_name = 'mrr'

dwh_tgt_incremental_column = 'updated_at'
dwh_tgt_table_name = 'ods_item_properties'
dwh_tgt_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = '/etl/mrr_item_properties_tag.csv'

file_path = '//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_11//increment//mrr_item_properties_tag.csv'
...
def update_DWH_ETL_log_start(dbcon,tablename):
    ...
    sql = """insert into DWH_ETL_log
             ...
             VALUES
             ((select max(cycle_id) from dwh_etl_cycle_list),CURRENT_TIMESTAMP,'mrr_item_properties_tag', 'load mrrtables','started running',1,'','');"""
    crr.execute(sql)
    dbcon.commit()
    crr.close()
...
def update_DWH_ETL_log_finish(dbcon, tablename):
    ...
    sql = """update DWH_ETL_log set end_time=CURRENT_TIMESTAMP , action_status='finished running' 
             WHERE job_name = 'mrr_item_properties_tag' 
