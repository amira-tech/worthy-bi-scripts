...
# Global parameters
src_table_name = "partners"
src_schema_name = "public"

tgt_table_name = "mrr_partners_tag"
tgt_schema_name = "mrr"

dwh_tgt_incremental_column = "updated_at"
dwh_tgt_table_name = "dim_partners"
dwh_tgt_schema_name = "public"

bucket_name = "worthy-etl"
s3_file_name = "/etl/mrr_partners_tag.csv"

file_path = "//home//ec2-user//scripts//Talend_2_python//Step_1_Source_To_Mirror_Part_1//increment//mrr_partners_tag.csv"
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    ...
    sql = """
            ...
            subscribed_new_auction,
            subscribed_daily_digest,
            remember_created_at,
            state_changed_at,
            "trusted",
            bid_approval_mandatory,
            "source",
            account_manager_id,
            email_reminders,
            fake,
...
def df_load_src_data(schema_name, table_name, updated_at, db_Connection):
    sql_qur = """
                ...
                credit_card_payer
                FROM {0}.{1} 
                WHERE updated_at > '{2}' and id not in (46832,46833,46834,46835,46838)""".format(schema_name.replace('\'', '\'\''), table_name.replace('\'', '\'\''), updated_at.replace('\'', '\'\''))
