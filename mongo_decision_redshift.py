...
# file from mongo
mongo_file = config.get('FILE_PATH', 'file')
mongo_file_bck =config.get('FILE_PATH', 'file_bck')
...
def update_DWH_ETL_log_start(dbcon,tablename):
    ...
    sql = """insert into DWH_ETL_log
             ...
             VALUES
             ((select max(cycle_id) from dwh_etl_cycle_list),CURRENT_TIMESTAMP,'ods_mongo_decisions_tag', 'load ods tables','started running',1,'','');"""
    crr.execute(sql)
    dbcon.commit()
    crr.close()
...
def copy_mrr_data_to_ods(dbcon, tablename):
    ...
    sql_param = 'createdAt'
    sql_create_table = 'CREATE TABLE ' + tablename + ' (LIKE mrr_mongo_decisions_tag);'
    sql_copy_data = 'insert into ' + tablename + ' select distinct * from mrr_mongo_decisions_tag ;'
    answr = checkTableExists(dbcon, tablename)
    if answr is True:
...
def update_DWH_ETL_log_finish(dbcon, tablename):
    ...
    sql = """update DWH_ETL_log set end_time=CURRENT_TIMESTAMP , action_status='finished running' 
             WHERE job_name = 'ods_mongo_decisions_tag' 
...
if __name__ == '__main__':
    ...
    # from mrr to ods
    table_name_tag = 'ods_mongo_decisions'
