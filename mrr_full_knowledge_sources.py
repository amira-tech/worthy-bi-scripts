...
## global params
src_table_name = 'knowledge_sources'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_full_knowledge_sources.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_full_knowledge_sources.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_knowledge_sources_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = f'SELECT id,	"name",	"desc",	created_at,	updated_at,	priority from {schema_table_name}'
    start = 0
    dfs = []
    # clear file if exist
    try:
