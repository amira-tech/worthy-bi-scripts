...
## global params
src_table_name = 'sms_templates'
src_schema_name = 'public'

bucket_name = 'worthy-etl'
s3_file_name = 'etl/mrr_sms_templates_tag.csv'

file_path = r'/home/ec2-user/scripts/Talend_2_python/Step_1_Source_To_Mirror_Part_11/full/mrr_sms_templates_tag.csv'
src_schema_table_name = src_schema_name+'.'+src_table_name

tgt_table_name = 'mrr_sms_templates_tag'
tgt_schema_name = 'mrr'
...
def read_sql_chunked(schema_table_name, conn, nrows, chunksize, file):
    # args
    sql = f'select id, sms_id, identifier, created_at, updated_at from sms_templates'
    start = 0
    dfs = []
    # clear file if exist
    try:
